...
static void b_disp() 
{
	...
	if(liststart != NULLREF) // Används om arrayen inte är tom
	{
...
static void b_add(int v) { 
	...
	if(numels >1) // Om fler än ett element finns i listan
	{
		...
		else if(v < get_value(liststart) || v == get_value(pcurr)) // Om talet skall ligga först eller före sin jämnlike
		{
			if(pcurr != liststart) // Om inte talet skall ligga på första platsen men ända är samma tal som pcurr 
			{	
				set_next(get_prev(pcurr), pnew);
				set_prev(pnew, get_prev(pcurr));
			}
